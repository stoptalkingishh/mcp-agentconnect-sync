...
The gateway's cloud transport is LiteLLM (`_call_via_litellm`, injectable via
`completion_fn` — see test_gateway_cloud.py), not the older raw-httpx
`_http_openai_compatible` this file originally mocked directly; these tests
go through that same `completion_fn` seam instead."""
...
def _fake_completion_response(content, pt=5, ct=5):
...
def test_gateway_notifies_failure_even_though_stub_masks_it():
    ...
    gw = ProviderGateway(secret_resolver=secrets, completion_fn=failing_completion, on_call_result=lambda *a: events.append(a))
...
def test_gateway_notifies_success():
    ...
    gw = ProviderGateway(secret_resolver=secrets, completion_fn=fake_completion, on_call_result=lambda *a: events.append(a))
...
def test_gateway_compresses_outbound_message_before_http_call():
    ...
    gw = ProviderGateway(secret_resolver=secrets, completion_fn=fake_completion, compressor=compressor)

    long_prose = "It is worth noting that " + ("padding " * 10)
    gw.call(_CFG, _req(long_prose))

    assert "It is worth noting that" not in captured["messages"][0]["content"]
...
def test_gateway_no_compressor_leaves_message_untouched():
    ...
    gw = ProviderGateway(secret_resolver=secrets, completion_fn=fake_completion)

    text = "It is worth noting that this stays untouched."
    gw.call(_CFG, _req(text))

    assert captured["messages"][0]["content"] == text
